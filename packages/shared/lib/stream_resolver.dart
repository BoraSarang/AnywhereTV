...
/// HLS master manifest 문자열에서 targetHeight 이상 최저 해상도 variant URL 추출 (순수 함수)
String? selectLowestVariant(String masterBody, int targetHeight) {
  if (!masterBody.contains('#EXTM3U')) return null;
...
Future<String?> _filterHlsLowestQuality(String masterUrl, {int targetHeight = 360}) async {
  ...
  try {
    ...
    final selected = selectLowestVariant(body, targetHeight);
    if (selected != null) {
      final bestUrl = Uri.parse(masterUrl).resolve(selected).toString();
      final resMatch = RegExp(r'RESOLUTION=(\d+)x(\d+)').firstMatch(body);
      final h = resMatch != null ? int.tryParse(resMatch.group(2)!) : null;
      log.info('HLS', 'selected ${h}p variant');
      return bestUrl;
    }
    return masterUrl;
  } catch (e) {
